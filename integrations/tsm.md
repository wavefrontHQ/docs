...
## Tanzu Service Mesh Setup
  ...
  **Prerequisites:**
  * While on-boarding new Kubernetes clusters on Tanzu Service Mesh dashboard, make sure to exclude the Wavefront Collector and Wavefront Proxy deployed namespaces.
    For example: `wavefront` and `wavefront-collector`.
  * When the Tanzu Service Mesh is configured as  multi-cluster, make sure to deploy Prometheus and Wavefront Collector on each cluster.
