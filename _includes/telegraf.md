...
1. Run the interactive configuration script: `bin/autoconf-wavefront-proxy.sh`. The script prompts you for the following properties:
   ...
   - *Authentication_Properties* - The authentication properties that you have to replace vary depending on whether your service is onboarded to the VMware Cloud Services platform. If Operations for Applications is **NOT** onboarded to the VMware Cloud Services platform, use a valid Operations for Applications API token. If your service **IS** onboarded to the VMware Cloud Services platform, use OAuth App credentials or an API token generated in the VMware Cloud Services Console. For the most recent instructions, see the steps on the **Setup** tab of the integration in the Operations for Applications user interface.
