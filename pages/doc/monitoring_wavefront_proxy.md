...
* On out-of-the-box dashboards that are based on proxy internal metrics, you can examine the health and usage of your proxies.
...
On the Proxies Browser, you can examine the details and statuses of the proxies in your Tanzu Observability service instance. A proxy can be in one of the following statuses:
...
On the Proxies Browser, click the name of a proxy to open its dashboard. The proxy dashboard consists of the following sections:
...
The Wavefront Usage integration includes the **Wavefront Service and Proxy Data** dashboard, which contains the **Proxies: Overview** and the **Proxy Troubleshooting** sections with charts for examining proxy health. You can navigate to this dashboard in two ways:
- Select **Dashboards > All Dashboards** and search for it.
- On the Proxies Browser, click **Usage and Proxies Data Dashboard**.

### Proxies Overview

This section of the **Wavefront Service and Proxy Data** dashboard includes a number of charts that show general information for all proxies as well as information per proxy, for example, the rate at which each proxy receives points, the rate at which each proxy sends points to Tanzu Observability, any queued or blocked points, and more.
