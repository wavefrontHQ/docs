...
Tanzu Observability by Wavefront supports two methods for sending log data metrics to Wavefront proxy: Filebeat and TCP. Wavefront proxy 4.4 and later supports these methods.
...
In this example, we configure Tanzu Observability by Wavefront to parse [Combined Apache Logs](http://httpd.apache.org/docs/2.4/logs.html#combined), which is a common logging format for many web services (for example, NGiNX). The example is merely a starting point&mdash; so you understand how to ingest metrics from any log format.
...
Through `valueLabel`, you can specify which part of the log line contains the metrics to send to Tanzu Observability by Wavefront. There are three supported modes of aggregation: counters, gauges, and histograms. See [Dropwizard documentation](http://metrics.dropwizard.io/3.1.0/getting-started/) for details.
...
In the example above, the log message has a timestamp. You can use this timestamp to match the message for ingestion, but Tanzu Observability by Wavefront always ingests the resulting metric at the time the Wavefront proxy sees the log, _not_ at the time the message was logged in your system. Therefore, the resulting metric would be: **myOperationDuration 42 &lt;proxy time&gt; source=&lt;host&gt;**
