...
To restrict memory usage of the container using Docker, you will need to add a JAVA\_HEAP\_USAGE environment variable and restrict memory using the `-m` or `--memory` options for the docker `run` command.  The container memory contraint should be at least 350mb larger than the JAVA\_HEAP\_USAGE environment variable.
