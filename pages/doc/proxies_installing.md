...
## Use the Proxies Browser

From the Proxies Browser, you can:

* Install or upgrade a proxy.
* See the status and the version of each proxy.
* Delete an orphaned proxy
* Find proxies that are actively sending data or stopped sending data.
* Check the backlog size of each proxy.
* Examine the performance of a proxy from its dedicated dashboard.
