...
You can now drill down to a corresponding trace from a topk chart and a histogram chart.

### Search for by Trace ID

You can now search for a trace or an operation by trace ID. This search does not always return results because Wavefront performs intelligent sampling, and because we keep trace data only for 7 days.
