...
* The **Show Node Counts** setting on the Application Map shows the number of instances of a service running on an application. For example, **shopping (5)** shows you that there are five instances of the shopping service on the beachshirts application.
...
* UI updates: Updates in the Wavefront taskbar, which is now keyboard accessible:
  ...
  * To navigate through the buttons of the taskbar, use the **Tab** key or the right and left arrow keys. 
  * To open a drop-down menu from the taskbar, select the item and press **Enter** or the down arrow. Then, use the down and up arrows to navigate through the list. To select an option, press **Enter**.
