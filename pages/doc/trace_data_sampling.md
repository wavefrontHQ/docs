...
{% include note.html content="A span that contains an error follows the rules for rate-based sampling and duration-based sampling in the same way as any other span. Therefore, the error tag is ignored unless you create a sampling policy to keep the spans that contain errors. Alternatively, you can use the intelligent sampling, which gives preference to traces that contain at least one span with an error." %}
