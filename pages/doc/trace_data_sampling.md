...
{% include note.html content="A span that contains an error, as well as any other span, follows the rules of the rate-based sampling and the duration-based sampling. Therefore, the error tag is ignored unless you create a sampling policy to keep the spans that contain errors." %}
