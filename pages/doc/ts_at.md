...
<td markdown="span">The period of time before `atTime` in which to look for a data value to return, if no data is reported at `atTime`. Specify a [time window](query_language_reference.html#query-elements) based on the clock or calendar (1s, 1m, 1h, 1d, 1w), or the window length (1vw) of the chart.
...
You designate the data value of interest by specifying the time it was reported. You can specify the report time: 
...
* As a time relative to your chart. For example, you can use `"start"` to obtain the data value that was reported at the beginning of your current chart window. In fact, if you're looking at a chart of past values, `"start"` causes `at()` to return the earliest of those past values.
...
If no data value corresponds to the specified report time, `at()` returns the last data value that was reported during the lookback period that immediately precedes the report time. By default, the lookback period is 1 hour. For example, suppose you run the query `at(2h, ts(my.metric))` at 8:00am, but `my.metric` did not report a data value at 6:00am. By default, this query returns the last data value that was reported between 5:00am and 6:00am. If no data was reported during that period, the query returns no data.

You can include a `lookbackWindow` parameter to specify a longer or shorter lookback period. For example: 
* A query such as `at(2h, 8h, ts(my.metric))` returns the last value to be reported during the period from 10 hours ago to 2 hours ago. 
* A query such as `at("end", 30s, ts(my.metric))` returns a data value only if one occurs at the end of your chart, and otherwise returns no data.
