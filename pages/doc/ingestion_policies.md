...
{% include note.html content="When you set a PPS limit for an ingestion policy and the PPS usage is higher than expected, no changes or implications occur. If the usage is higher than expected, for example, if a team uses more PPS than the PPS limit set for an ingestion policy, you can increase the PPS limit or provide a training for the users who ingest more datacd c. The system doesn't block or prevent certain users or groups from ingestion more data. "%}
...
<td>Shows the total usage out of the committed usage - the usage for the selected month out of the PPS limit set for the ingestion policy.</td></tr>
<tr>
