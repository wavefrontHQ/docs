...
- **Source names** should reflect a unique entity that is emitting metrics. When Wavefront shows you the full list of sources, performs queries, etc, Wavefront assumes that source names are unique.
  ...
  When it is not clear which tag to use as the source tag, use the most unique value for a source tag. For higher-level pre-aggregated data, for example, a datacenter-wide metric like power usage, you can use the name of the datacenter.
