...
* **Logs (Beta)**:
  ...
  - Improved the search time window when you [drill down](logging_drill_into_logs.html#traces-browser) from the Traces browser into the related logs. By default, the search time window now starts 5 seconds before the trace and ends 5 seconds after the trace, but it's customizable.
