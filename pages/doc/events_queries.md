...
The `events()` function returns the set of stored [events](events.html) that match one or more event filters. You can use `events()` to display events in time-series charts, for example, to inform other users about reasons for a change in a time series.

You can describe the set of events to be returned using any combination of [event filters](#filters). You can filter on user event characteristics, such as event type, severity, and tags. You can include alert characteristics, such as alert tags and alert ID, to match events that are associated with alerts. You can combine multiple event filters using Boolean operators. For example:

* The following equivalent queries display user events or events based on any alerts that start with `Request` and are either severity `severe` or `warn`:
  - `events(name="Request*" and (severity="severe" or severity="warn"))`
  - `events(name="Request*", severity="warn" or severity="severe")`

* The following query displays events that either have severity `warn` or that are generated by the source `app-1`:
  - `events(severity="warn" or source="app-1")`

You can use `events()` as an input `eventsExpression` that you specify to another [event function](query_language_reference.html#event-functions). 

You can use `events()` as input expressions (`e1`, `e2`) to [event set operators](#event-set-operators) that combine and compare event sets.

<!---
Add subsection summarizing main categories of event functions 
--->
