...
summary: Create and manage server-to-server apps in VMware Aria Operations for Applications on VMware Cloud services.
---
...
{% include note.html content="A server-to-server app must have **permissions** to perform tasks. To run queries, a server-to-server app must have the **Metrics** permission. To manage dashboards and alerts, the server-to-server app might need both permissions and [access](access.html). " %}
...
As a user with the **Accounts** permission, you [generate (and revoke, if needed)](wavefront_api.html#generate-and-manage-the-api-tokens-for-a-service-account) authentication tokens for the service account. It’s also possible to deactivate a service account completely. 
