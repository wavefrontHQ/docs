...
When used as an aggregation function, returns the highest value across the set of time series described by `expression`. The results are computed from interpolated values and real reported values. Use [`rawmax()`](ts_rawmax.html) if you don't need interpolation.
