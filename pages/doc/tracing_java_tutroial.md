...
1. Configure the application to send data to Wavefront via the Wavefront proxy.
    1. <a href="proxies_installing.html#proxy-installation">Install the Wavefront proxy</a>. For this tutorial let's install the Wavefront Proxy on Docker.
        ...
        {%include tip.html content="If you are not sure of your cluster name or token, open the Wavefront application UI, click **Browse** > **Proxies** > **Add New Proxy**, and click the **Docker** tab. Copy the values for `WAVEFRONT_URL` and `WAVEFRONT_TOKEN`, and replace the values in the above example."%}
...
1. Click **Applications** > **Application Status** to get an overview of the services and applications.
  ...
  * [**Table View**](tracing_ui_overview.html#table-view) gives you a list the applications and services. You can see the Request, Error, and Duration (RED) metrics at a glance and sort the data.
  * [**Grid view**](tracing_ui_overview.html#grid-view) lists the application and services in a grid. You can see the RED metrics for each of the application’s services.
  ![The screenshots shows the beachshirts and foo-beachshirts application on the app map view.](images/tracing_java_tutorial_application_status.png)
