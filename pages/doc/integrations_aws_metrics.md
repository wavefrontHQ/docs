...
Tanzu Observability by Wavefront automatically sets each metric's source field and adds source tags to each AWS source, as follows:

**Metric Source Field**

Tanzu Observability by Wavefront sets the value of the AWS metric [`source`](wavefront_data_format.html) field by service:
...
Tanzu Observability by Wavefront adds the following point tags to CloudWatch metrics:
...
Tanzu Observability by Wavefront reports the single metric `aws.billing.estimatedcharges`. The `source` field and `ServiceName` point tag identify the AWS services. For the total estimated charge metric, `source` is set to `usd` and `ServiceName` is empty. Tanzu Observability by Wavefront also provides the point tags `accountId`, `Currency`, `LinkedAccount`, and `Region`. Billing metrics are typically reported every 4 hours.
...
We retrieve CloudTrail event information stored in JSON-formatted log files in an S3 bucket. The CloudTrail integration parses the files for all events that result from an operation that is not a describe, get, or list, and creates a Tanzu Observability by Wavefront [System event](events.html).
...
Unless otherwise indicated, Tanzu Observability by Wavefront sets the value of the AWS Metrics+ `source` field to the AWS instance ID. If an EBS volume is detached, its source field is set to the volume ID. The metrics include:
...
- Service Limit Metrics - capture the current resource limits and usage for your AWS account. These metrics include the point tags `Region` and `category`.
  ...
  - `aws.limits.<resource>.usage` - the current usage of an AWS resource in a particular region.
    ...
    {% include note.html content="To examine these metrics, your account needs at least the Business-level AWS Support plan because the integration uses the Support API to pull service limits. You also need both ReadOnlyAccess and AWSSupportAccess. See [Giving Tanzu Observability by Wavefront Read-Only Access](integrations_aws_overview.html#giving-tanzu-observability-by-wavefront-access-to-your-aws-account) for details." %}
