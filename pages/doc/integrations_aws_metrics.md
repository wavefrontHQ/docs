...
You must have [Proxy Management permission](permissions_overview) to set up an AWS cloud integration. If you do not have permission, UI menu selections and buttons required to perform the tasks will not be visible.

Wavefront offers [Amazon Web Services](http://aws.amazon.com) (AWS) [CloudWatch](http://aws.amazon.com/cloudwatch), [CloudTrail](http://aws.amazon.com/cloudtrail), and AWS API integrations.
...
-   **Register [CloudWatch \| CloudTrail \| AWS APIs]** - Integrate each type of data individually. Choose this option if:
    ...
    -   You want to integrate CloudWatch, CloudTrail, and AWS API data into Wavefront, but not at the same time.
...
2.  Select **Add Integration &gt; &lt;Integration Option&gt;**, where  **&lt;Integration Option&gt;** is **Set up Amazon Account** or **Register [CloudWatch \| CloudTrail \| AWS APIs]**.
3.  Configure the integration:
    ...
    -   **Register [CloudWatch \| CloudTrail \| AWS APIs]**
...
The AWS APIs integration retrieves additional metrics using AWS API calls other than CloudWatch:
...
Wavefront sets the value of the AWS APIs metric **source** field to the AWS instance ID.
