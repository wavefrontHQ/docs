...
* **Insights Integration**: If your VMware Cloud organization is running also the VMware Tanzu Insights service on VMware Tanzu Hub, there's a bidirectional link between your Operations for Applications and Insights services. Operations for Applications sends alerts data to Insights. Insights correlates the received alerts with other events from multiple sources and aggregates them into insights, which sends back to Operations for Applications. The Operations for Applications user interface shows the insights that are related to each alert. From the Operations for Applications user interface, you can directly open a related insight in VMware Tanzu Hub and inspect the complete list of alerts and events aggregated into that insight.
