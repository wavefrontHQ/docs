...
* **Alerts with Related Insights**: If your VMware Cloud organization is running also VMware Tanzu Insights, there's a bidirectional link between Operations for Applications and Insights. Operations for Applications sends alerts data to Insights. Insights correlates the received alerts with other events from multiple sources and aggregates them into insights. The Alerts Viewer and Alerts Editor in Operations for Applications show the insights that are related to the corresponding alert. From the Operations for Applications user interface, you can also directly open a related insight in VMware Tanzu Hub and inspect the complete list of alerts and events aggregated into that insight.